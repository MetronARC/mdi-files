...
abstract class BaseController extends Controller
{
    ...
    /**
     * Helper method to check if the current page matches a given route
     */
    protected function isActive($route): bool
    {
        $currentPath = trim(current_url(true)->getPath(), '/');
        $basePath = trim(parse_url(base_url(), PHP_URL_PATH), '/');
        
        // Remove the base path from current path if it exists
        if (strpos($currentPath, $basePath) === 0) {
            $currentPath = trim(substr($currentPath, strlen($basePath)), '/');
        }
        
        // For the root path
        if (empty($currentPath) && $route === '/') {
            return true;
        }
